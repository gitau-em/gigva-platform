...
function calcPayroll(basic, house, car, other, pension=0, mi=0) {
  ...
  return (
    <div id="payslip-print-area" style={{ fontFamily: 'Arial, sans-serif', fontSize: '13px', color: '#222', padding: '24px', maxWidth: '800px', margin: '0 auto', backgroundColor: '#fff' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
        <thead>
          <tr>
            <th style={hd}>Item</th><th style={hd}>Code</th>
            <th style={{ ...hd, width: '35%' }}>Name</th>
            <th style={hd}>Qty/Rate</th><th style={hd}>Amount</th><th style={hd}>Total</th>
          </tr>
        </thead>
        <tbody>
          {items.map((it, i) => (
            <tr key={it.code} style={{ backgroundColor: i % 2 === 0 ? '#f0f5ff' : '#fff' }}>
              ...
              <td style={{ ...td, textAlign: 'right' }}>1.00</td>
              <td style={{ ...td, textAlign: 'right' }}>{fmtKsh(it.amount)} KSh</td>
              <td style={{ ...td, textAlign: 'right', fontWeight: it.code==='NET_PAY'?'bold':'normal', backgroundColor: it.code==='NET_PAY'?'#e8f0fe':'inherit' }}>
                {fmtKsh(it.amount)} KSh
              </td>
            </tr>
          ))}
        </tbody>
