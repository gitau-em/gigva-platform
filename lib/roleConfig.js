...
export const ROLES = {
  superadmin: {
    ...
    tabs:         ['messages', 'demos', 'trials', 'users'],
  },
  ceo: {
    label:        'CEO',
  ...
  product: {
    ...
    description:  'Product feedback — demo requests, feature interest',
  ...
  people_ops: {
    label:        'People & Ops',
    description:  'HR, hiring, and people operations',
...
/** Derive a stable role key from a user object (prefers explicit role field). */
export function resolveRole(user) {
  if (!user) return null
  if (user.is_admin) return 'superadmin'
